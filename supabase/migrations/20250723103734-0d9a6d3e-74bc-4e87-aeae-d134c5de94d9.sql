...
DECLARE
  ...
  ad_is_premium boolean;
  ad_top_spot boolean;
  ad_top_spot_until timestamp with time zone;
  ad_times_shown integer;
  user_membership text;
  score numeric := 0;
  hours_since_created numeric;
  boost_multiplier numeric := 1;
BEGIN
  -- جلب بيانات الإعلان والمستخدم
  SELECT 
    a.created_at,
    a.is_premium,
    a.top_spot,
  ...
  hours_since_created := EXTRACT(EPOCH FROM (now() - ad_created_at)) / 3600;
  ...
  IF ad_is_premium THEN
    score := score * 1.3;
  END IF;
  
  -- مضاعف كبير للإعلانات في Top Spot النشطة
  IF ad_top_spot AND ad_top_spot_until > now() THEN
    -- تقليل النقاط بناءً على عدد مرات العرض للتدوير العادل
    boost_multiplier := GREATEST(10 - (ad_times_shown * 0.5), 5);
