...
serve(async (req) => {
  ...
  try {
    ...
    const visionResponse = await fetch(
      `https://vision.googleapis.com/v1/images:annotate?key=${serviceAccount.private_key_id}`,
      {
        method: 'POST',
        headers: {
    ...
    const amountPattern = /25000|٢٥٠٠٠/
    ...
    if (!hasAccountNumber || !hasBeneficiary || !hasAmount || !membershipIdMatch) {
      const missingData = []
      ...
      if (!membershipIdMatch) missingData.push('رقم العضوية')
