...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { ad_id, hours_duration = 24 } = await req.json();
      ...
      // ترقية الإعلان
      const { data: result, error } = await supabaseClient.rpc('boost_ad_to_top_spot', {
        ad_id_param: ad_id,
      ...
      if (error) {
        ...
        return new Response(
          JSON.stringify({ error: "فشل في ترقية الإعلان" }), 
          { 
            headers: { ...corsHeaders, "Content-Type": "application/json" }, 
            status: 500 
          }
    ...
    if (req.method === 'GET') {
      ...
      // فحص إمكانية الترقية
      const { data: canBoost, error } = await supabaseClient.rpc('can_boost_ad', {
        ad_id_param: ad_id,
        user_id_param: user.id
      });
